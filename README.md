<p align="center">
  <img src=".github/assets/spectemus-simul-banner.svg" alt="Spectemus Simul — приватный совместный просмотр" width="100%" />
</p>
...
человеком. Сегодня это работает в одной домашней сети: вы запускаете
приложение на компьютере host-а, создаёте комнату и отправляете ссылку или
QR-код. Гость открывает ссылку в браузере — видео, звук и управление
воспроизведением синхронизируются.
...
Главная цель Spectemus Simul — чтобы вы могли включить свой фильм у себя дома,
отправить приватное приглашение близкому человеку в другом городе и посмотреть его
синхронно. LAN preview — первый надёжно проверяемый шаг к этой цели, а не
окончательная граница продукта.

| Этап                                   | Что это даст                                                                                                                               |
| -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| **Сейчас: LAN preview**                | Host и гости в одной сети, локальный файл, ссылка/QR-код, синхронный просмотр, чат и восстановление показа.                                |
| **Следом: закрытая Internet alpha**    | Два человека из разных городов, защищённые аккаунты и отзывные приглашения, HTTPS/TURN для сложных сетей и диагностика соединения.         |
| **Затем: стабильный Internet release** | Подписанные приложения, понятная установка, проверенная работа на разных сетях и контролируемые лимиты небольшой приватной комнаты.        |
| **Параллельно: больше видеоформатов**  | Desktop host локально подготовит совместимую временную копию для распространённых форматов и кодеков, не загружая исходный фильм в сервис. |

Мы не называем календарную дату Internet release до реальных проверок. Перед
первым внешним приглашением должны пройти сессии из двух разных сетей с
видео, звуком, чатом и reconnect, включая сеть с заблокированным UDP. Также
нужны HTTPS/TURN, безопасные аккаунты и отзыв приглашений. Это делает режим
между городами не «ссылкой на удачу», а приватным и предсказуемым просмотром.

Расширение форматов тоже идёт поэтапно. Базовые MP4/M4V и WebM приложение
сначала проверяет напрямую. Для других контейнеров, а также если такая проверка
не прошла, desktop host создаёт временную совместимую MP4-копию локально и
удаляет её после просмотра. Оригинал не будет загружаться или храниться
сервисом. DRM-защищённые стриминги, каталог фильмов, торренты и обход
ограничений не входят в этот план.
...
гарантированной поддержки каждого старого или редкого видеофайла. Это цели
следующих этапов, описанных выше, а не скрытые обещания текущей preview.
...
| MOV, MKV, AVI, MPEG, TS, WMV, FLV            | Desktop host готовит временный совместимый MP4: уже подходящие H.264/AAC дорожки быстро копируются без потери качества, а несовместимые кодеки перекодируются локально.         |
| HEVC, DTS, редкие кодеки, повреждённые файлы | Распространённые HEVC/DTS-файлы — цель встроенной подготовки, но результат зависит от состояния файла и H.264 encoder-а компьютера. DRM и повреждённые файлы не поддерживаются. |
...
| Фильм не публикуется          | Дождитесь локальной подготовки совместимой копии. Если она не удалась, попробуйте H.264/AAC MP4: исходный файл при этом не меняется и никуда не отправляется. |
