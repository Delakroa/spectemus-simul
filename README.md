...
| **Параллельно: больше видеоформатов**  | Локальная подготовка совместимой временной копии для распространённых форматов и кодеков, без загрузки исходного фильма в сервис.   |
...
Расширение форматов тоже идёт поэтапно. Уже сейчас приложение честно проверяет
на устройстве host-а MOV, MKV и другие контейнеры, но не обещает их успех.
Следующий шаг — local normalizer: он сможет подготовить поддерживаемый файл во
временную копию на компьютере host-а и удалить её после просмотра. Оригинал не
будет загружаться или храниться сервисом. DRM-защищённые стриминги, каталог
фильмов, торренты и обход ограничений не входят в этот план.
