...
import com.spectemus.simul.backend.config.TimeConfig;
...
@TestPropertySource(properties = "spring.application.name=spectemus-simul-backend")
class SystemControllerTest {
    ...
    void versionEndpointReturnsApplicationVersion() throws Exception {
        mockMvc.perform(get("/api/v1/version"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.name").value("spectemus-simul-backend"))
