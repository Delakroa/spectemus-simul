...
@ConfigurationProperties("spectemus-simul.rooms")
public record RoomProperties(Duration ttl, boolean sessionCookieSecure, Duration cleanupGrace) {
    ...
    public RoomProperties {
        if (ttl == null || ttl.isZero() || ttl.isNegative()) {
            throw new IllegalArgumentException("spectemus-simul.rooms.ttl must be positive");
        }
        ...
        if (cleanupGrace.isNegative()) {
            throw new IllegalArgumentException(
                    "spectemus-simul.rooms.cleanup-grace must be zero or positive");
        }
    }
