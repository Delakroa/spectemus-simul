...
- Frontend хранит до 200 последних сообщений в памяти сессии (`chatMessages`), в хронологическом порядке. История эфемерна: не входит в `room.snapshot`, теряется при перезагрузке страницы, но переживает auto-reconnect room WebSocket в той же вкладке.
...
- История чата эфемерна и хранится только в памяти клиента: auto-reconnect WebSocket сохраняет её, но перезагрузка страницы, повторный join или сообщения, пришедшие во время кратковременного офлайна, не восстанавливаются (нет replay в `room.snapshot`).
