...
## Именование тикетов, веток и коммитов

Префикс тикета — `SPS-` (Spectemus Simul). Нумерация сквозная и продолжает прежний ряд.

- ветка: `SPS-692--host-preview-mute`
- коммит: `SPS-692 Не включать заново выключенный звук preview`
- отчёт: `docs/SPS-692_HOST_PREVIEW_MUTE.md`

Отчёты с префиксом `WT-` остаются историей проекта до ребрендинга и не переименовываются: на них ссылаются git history, номера PR и перекрёстные ссылки между документами. Новый префикс применяется только к тикетам начиная с 692.
