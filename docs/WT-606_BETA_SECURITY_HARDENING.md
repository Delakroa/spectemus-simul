...
- Дефолтные бюджеты (на IP): create 10/мин, join 20/мин, livekit-token 30/мин, feedback 10/мин, telemetry 60/мин. Настраиваются через `spectemus-simul.rate-limit.*` (env `RATE_LIMIT_*`). Вся фича выключается флагом `RATE_LIMIT_ENABLED=false` (по умолчанию включена — secure by default).
...
- `backend/.../ratelimit/RateLimitProperties.java` — `@ConfigurationProperties("spectemus-simul.rate-limit")` с бюджетами по bucket и флагом enabled.
...
- `backend/src/main/resources/application.yml` — блок `spectemus-simul.rate-limit`.
