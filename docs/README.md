...
Пользовательскую установку и сценарий первого просмотра ищите в
[корневом README](../README.md). Эта папка предназначена для разработки,
архитектуры и проверки решений.
...
- [WT-672_DESKTOP_HEALTH_WITHOUT_REDIS.md](WT-672_DESKTOP_HEALTH_WITHOUT_REDIS.md) — desktop host запускается без Docker и Redis.
- [WT-673_DESKTOP_PUBLIC_INVITE_ORIGIN.md](WT-673_DESKTOP_PUBLIC_INVITE_ORIGIN.md) — Electron показывает host UI через loopback, но создаёт LAN invitation с выбранным Ethernet-адресом.
...
- [WT-687_BACKEND_SESSION_CONSISTENCY.md](WT-687_BACKEND_SESSION_CONSISTENCY.md) — trusted proxy не отдаёт rate limit под контроль клиента, а desktop room сохраняет lifecycle при reconnect и WebSocket-гонке.
...
- [SPS-715_ADAPTIVE_MEDIA_NORMALIZATION.md](SPS-715_ADAPTIVE_MEDIA_NORMALIZATION.md) — MKV и другие контейнеры проходят быстрый remux или частичное перекодирование вместо безусловной полной конвертации.
