...
Страница guest после подписки показывает:
...
Пока не форсируем 1:1. Для MVP важнее стабильность, отсутствие packet loss, приемлемый first frame и поведение при 1/2/3 guests. Возврат к 1080p one-to-one возможен отдельной настройкой encoder parameters, bitrate и viewport, если метрики покажут, что это нужно и не ломает стабильность.
...
| Host reconnect             | Host может reconnect и republish                    | PASS      | Есть auto-republish после LiveKit reconnect/unpublish событий; ручной `Publish` остаётся fallback. |
...
`GO` для фиксации WT-004 media pipeline ADR и продолжения product-state прототипа: media path стабилен на 1 guest, звук работает, packet loss равен 0, long-run 27 минут подтверждён.
