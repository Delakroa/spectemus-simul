...
2. В Finder дважды нажмите `Start-Spectemus-Simul.command` в корне репозитория.
...
нужны на компьютере host-а. Следующий продуктовый этап для людей без терминала
и Docker — отдельное desktop-приложение с включённым runtime и понятным
install/update flow; он требует отдельного архитектурного решения и не
подменяется этим launcher-ом.
